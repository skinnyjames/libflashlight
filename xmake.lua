...
package("libdill")
  ...
  if is_kind("static") then
    ...
    on_install("linux", "macosx", function(package)
      os.vrun("./autogen.sh")
      os.vrun("./configure %s %s --prefix=\"%s\"", "--disable-sockets --enable-valgrind", is_mode("debug") and "--enable-valgrind" or "", package:installdir())
      os.vrun("make")
      os.vrun("make check")
      os.vrun("make install")
    end)
...
target("lib")
  ...
  --   local dill = target:pkg("libdill")
  --   local pcre = target:pkg("pcre2")
  --   local libdill = path.join(dill:installdir(), "lib", "libdill.a")
  --   local libpcre = path.join(pcre:installdir(), "lib", "libpcre2-posix.a")
  --   printf("%s\n", libdill)
  --   merge_staticlib(target, target:filename(), {"/home/skinnyjames/src/libflashlight/libdill.a", libpcre})
  -- end)
  -- after_install(function (target)
  --   local dill = target:pkg("libdill")
  --   local libdillh = path.join(dill:installdir(), "include", "libdill.h")
  --   os.cp(libdillh, "/usr/local/include")
  -- end)
