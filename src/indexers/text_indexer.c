...
coroutine void f_index_text_bytes(int fd, int done, f_indexer_chunk* ic, int thread)
{
  ...
  // f_log(F_LOG_ERROR, "count: %zu - [%u] from: %zu to: %zu\n", ic->count, ic->index, ic->from, ic->from + ic->count);
...
f_index* f_index_text_file(f_indexer indexer)
{
  ...
  f_log(F_LOG_ERROR, "max bytes per iteration %zu, thread it count: %d", max_bytes_per_iteration, thread_it_count);
  ...
  for (int itc=thread_it_count - 1; itc>=0; itc--)
  {
    ...
    {
      // f_log(F_LOG_ERROR, "truncating max bytes per iteration (before: %zu)", max_bytes_per_iteration);
      local_max_bytes_per_iteration = total_bytes_count - thread_it_start;
      // f_log(F_LOG_ERROR, "truncating max bytes per iteration (%zu) %lu %zu", max_bytes_per_iteration, thread_it_start, total_bytes_count);
    }
    ...
    */
    ...
    for (int i=0; i<it->len; i++)
    {
      ...
      // f_log(F_LOG_ERROR, "[iteration %d] Starting thread %d", itc, i);
      // f_log(F_LOG_ERROR, "[%d] total bytes: %zu, %zu - from: %zu, to: %zu - concurrency: %d", i, total_bytes_count, local_max_bytes_per_iteration, tthread->from, tthread->to, tthread->concurrency);
    ...
    */
    ...
    // f_log(F_LOG_ERROR, "last? %d, init? %d", last_lookup, init_lookup);
